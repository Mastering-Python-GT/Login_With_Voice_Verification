...
class AVerefication(UserControl):
    ...
    def create(self):
        ...
        def login(e):
            ...
            if status == True:
                kk.value = ''
                gg.content = ft.ProgressRing(width=25, height=25, stroke_width = 2)
                self.page.update()
                write('test.mp3', t.args.samplerate, c)
                model = Model.from_pretrained(
                    "pyannote/embedding", use_auth_token="get_access_token_from_huggingface")
                inference = Inference(model, window="whole")
                filename = self.page.data[0]+".mp3"
                embedding1 = inference(filename)
                embedding2 = inference('test.mp3')
                x = [embedding1, embedding2]
                distance = pdist(x, metric="cosine")[0]
                if distance < 0.1:
                    self.page.go("/welcome")
                else:
                    kk.value = "Non-Identical Voice"
                    self.page.update()
                os.remove("test.mp3")
                gg.content = rr
                self.page.update()
                                ...
                                ft.Container(
                                    content=ft.Column(
                                        ...
                                        controls=[
                                            ...
                                            ft.Text(
                                                "Account Verefication", font_family="Poppins", size=20, weight='bold', color='#28282B')
